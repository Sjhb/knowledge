...
- 带宽：单位时间能通过链路的数据量。通常以bps来表示，即每秒可传输字位数
- 延迟：
...
- 服务端推送（server push），HTTP2.0也具有server push功能。例如我的网页有一个sytle.css的请求，在客户端收到sytle.css数据的同时，服务端会将sytle.js的文件推送给客户端，当客户端再次尝试获取sytle.js时就可以直接从缓存中获取到，不用再发请求了

### http/1.0/1.1/2.0 在并发请求上的主要区别是什么
1. http/1.0
    每个TCP连接只能发送一个请求，当服务器响应后就会关闭这个连接，下一次请求需要再次建立TCP连接。
2. http/1.0
    默认使用持久连接，TCP，Connecttion:keep-alive,通过Connnection:close可以设置关闭持久连接
    增加了管道机制，具体为在同一个TCP连接里，允许多个请求同时发送。但是是串行的，因为TCP连接是有顺序的，服务端响应也是有顺序的，就算客户端同时发送了多个请求
3. http/2.0
    增加双工模式：服务端能同时处理多个请求，客户端可以同时发送多个请求
    增加多路复用：没有次序的概念了
    增加服务器推送功能

### HTTP/1.1的长连接和2.0的多路复用有什么区别
1.1: 一个TCP连接同一时间只能处理一个请求，一问一答的形式。
追问：为什么chrome支持最大6个同域请求的并发
因为chrome支持最大6个TCP连接
2.0： 同域名上的所有通信都在单个连接上完成，单个连接上可以并行交错的进行请求和响应

### HTTP/1.1为什么不能实现多路复用
传输数据的方式不同
HTTP/2.0是基于二进制帧的协议，HTTP/1.1基于文本分隔解析的协议
1.1的报文结构里，服务器需要不断的读入字节，直到遇到换行符，处理的顺序是串醒的
2.0以帧为最小数据单位，每个帧都会有标识自己术语哪个流，多个帧组成一个流
多路复用，其实就是一个TCP存在多个流
...
> 参考: https://juejin.cn/post/6844903489596833800#comment

### osi网络模型
一种概念模型
- 第7层 应用层，HTTP/HTTPS/FTP/SSH
- 第6层 表示层
- 第5层 会话层
- 第4层 传输层 TCP/UDP
- 第3层 网络层 IP
